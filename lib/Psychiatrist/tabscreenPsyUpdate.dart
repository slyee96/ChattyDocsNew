...
class _UpdatePscyhiatristScreenState extends State<UpdatePscyhiatristScreen> {
  ...
  void uploadData(){ 
    ...
    http.post(urlUpdatePsychiatrist, body: {
      ...
      if (res.body == "success") {
        ...
        pr.hide();
        _onLoginAfterUpdate(widget.psychiatrist.psychiatristID, context);
      } else {
  ...
  void _onLoginAfterUpdate(String psychiatristID, BuildContext ctx) {
    String urlgetuser = "http://myondb.com/latestChattyDocs/php/getPsychiatrist.php";
    print("get data");
    http.post(urlgetuser, body: {
      "username": psychiatristID,
    }).then((res) {
      ...
      if (dres[0] == "success") {
        Psychiatrist psychiatrist = new Psychiatrist(
                ...
                location: dres[10]);
        Navigator.push(ctx,
            MaterialPageRoute(builder: (context) => MainScreenPsychiatrist(psychiatrist: psychiatrist)));
      }
    }).catchError((err) {
      print(err);
    });
